    ...
    stages {
        ...
        stage('Deliver (Linux)') {
            ...
            steps {
                dir("sources") {
                    sh 'rm -rf ./dist'
                    sh 'rm -rf ./build'
		    sh 'pyinstaller backend.py --hidden-import bottle_websocket --add-data "eel.js:eel_for_transcrypt" --add-data "web/**:web" --add-data "web/additions_diary.html:web" --add-data "web/__target__/frontendscrypt.js:web/__target__" --add-data "web/__target__/org.transcrypt.__runtime__.js:web/__target__" --add-data "web/__target__/frontendscrypt.options:web/__target__" --onefile'
		...
		steps {
			dir("sources") {
				...
				sh 'pyinstaller backend.py -n backend.exe --hidden-import bottle_websocket --add-data "eel.js;eel_for_transcrypt" --add-data "web/**;web" --add-data "web/additions_diary.html;web" --add-data "web/__target__/frontendscrypt.js;web/__target__" --add-data "web/__target__/org.transcrypt.__runtime__.js;web/__target__" --add-data "web/__target__/frontendscrypt.options;web/__target__" --onefile 2>&1 | cat'
